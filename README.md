...
- 📖 法术/战技/程序/魔法物品词典（`dict.html`）：浏览/搜索/筛选全部 **1127 个法术**（含完整属性与全文、职业法表反查、规则书原文链接）、**252 个战技**（流派×级别×类型）、**226 个程序**（协议层级×模块）、**约 1550 个魔法物品**（分类/同调/神器筛选、内嵌价格表、原文链接），数据与车卡同源（构建时从规则书自动解析），永远与规则书同步
